...
def mark_navigation_phase(updated_dataframe):
...
def mark_planning_phase(dataframe):
    phase_list = []
    planning_flag = False

    for index, row in dataframe.iterrows():
        if row['taskState'] == 'taskAccepted' or 'mapLog:MapButtonPoiClick+buttonOnStopNavigation' in row['mapInteractions']:
            planning_flag = True
        elif row['phase'] == 'navigation':
            planning_flag = False

        if planning_flag:
            phase_list.append('planning')
        else:
            phase_list.append(row['phase'])

    dataframe['phase'] = phase_list
    return dataframe
